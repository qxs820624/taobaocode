...
def edit_wiki(request, name, path=None):
    ...
    rc = request.rc
    rc.project = project
    rc.navmenusSource = build_prj_nav_menu(request, project, 'wiki')
...
def raw_wiki(request, name, path):
    ...
    resp = HttpResponse(wiki.content, content_type="text/plain")
...
def raw_log_wiki(request, name, logid):
    ...
    resp = HttpResponse(log.content, content_type="text/plain")
    resp['X-Content-Type-Options'] = 'nosniff'
    resp['X-XSS-Protection'] = '1; mode=block'
